...
class Order : JSONInitializable {
    ...
    init(id: Int, number: Int, name: String, email: String, totalPrice: Double) {
        self.id             = id
        self.number         = number
        self.name           = name
        self.email          = email
    ...
    required init?(json: JSON) {
        let id      = json["id"].int
        let number  = json["order_number"].int
        let name    = json["name"].string
        let email   = json["email"].string
        let price   = json["total_price_usd"].string
        
        if let id = id, let number = number, let name = name, let email = email, let price = price {
            self.id                 = id
            self.number             = number
            self.name               = name
            self.email              = email
            ...
            if let financialStatus = json["financial_status"].string {
                ...
                print("Order #\(self.number) is not paid!")
            }
        } else {
            return nil
        }
